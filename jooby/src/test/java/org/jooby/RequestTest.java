...
public class RequestTest {
  ...
  public void xhr() throws Exception {
    new MockUnit(Mutant.class)
        .expect(unit -> {
          ...
          when(xRequestedWith.toOptional(String.class)).thenReturn(Optional.of("XMLHttpRequest"), Optional.empty());
        })
        .run(unit -> {
  ...
  public void path() throws Exception {
    new MockUnit(Route.class)
        .expect(unit -> {
          ...
          when(route.path()).thenReturn("/path");
        })
        .run(unit -> {
  ...
  public void verb() throws Exception {
    new MockUnit(Route.class)
        .expect(unit -> {
          Route route = unit.get(Route.class);
          when(route.method()).thenReturn("PATCH");
        })
        .run(unit -> {
