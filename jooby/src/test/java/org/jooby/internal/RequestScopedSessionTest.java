...
public class RequestScopedSessionTest {
  ...
  private MockUnit.Block cookie = unit -> {
    ...
    when(unit.get(SessionManager.class).cookie()).thenReturn(cookie);

    when(cookie.maxAge(0)).thenReturn(cookie);
  ...
  private MockUnit.Block isDestroyed(boolean destroyed) {
    return unit -> {
      when(unit.get(SessionImpl.class).isDestroyed()).thenReturn(destroyed);
    };
  }
