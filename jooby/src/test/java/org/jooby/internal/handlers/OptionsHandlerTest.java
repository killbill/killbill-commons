...
public class OptionsHandlerTest {
  ...
  public void handle() throws Exception {
    new MockUnit(Request.class, Response.class, Route.Chain.class, Route.Definition.class)
        ...
        .expect(unit -> {
          ...
          when(rsp.header("Allow", "")).thenReturn(rsp);
  ...
  public void handleSome() throws Exception {
    new MockUnit(Request.class, Response.class, Route.Chain.class, Route.Definition.class)
        ...
        .expect(routeMethods("POST", "PATCH"))
        .expect(matches("PUT", false))
        .expect(matches("DELETE", false))
        ...
        .expect(unit -> {
          Response rsp = unit.get(Response.class);
          when(rsp.header("Allow", "POST, PATCH")).thenReturn(rsp);
          when(rsp.length(0)).thenReturn(rsp);
          when(rsp.status(Status.OK)).thenReturn(rsp);
        })
        .run(unit -> {
  ...
  private Block matches(final String method, final boolean matches) {
    return unit -> {
      ...
      when(def.matches(method, "/", MediaType.all, MediaType.ALL)).thenReturn(ifRoute);
    };
  }
  ...
  private Block routeMethods(final String... methods) {
    return unit -> {
      Route.Definition req = unit.get(Route.Definition.class);
      when(req.method()).thenReturn(methods[0],
          java.util.Arrays.copyOfRange(methods, 1, methods.length));
    };
  }

  private Block routeMethod(final String method) {
    return routeMethods(method);
  }

  private Block allow(final boolean set) {
    return unit -> {
      Mutant mutant = unit.mock(Mutant.class);
      when(mutant.isSet()).thenReturn(set);

      Response rsp = unit.get(Response.class);
      when(rsp.header("Allow")).thenReturn(mutant);
    };
  }
