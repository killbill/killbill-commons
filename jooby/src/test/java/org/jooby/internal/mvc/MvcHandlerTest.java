...
public class MvcHandlerTest {
  ...
  public void handle() throws Exception {
    ...
    new MockUnit(RequestParamProvider.class, Request.class, Response.class, Route.Chain.class)
        .expect(unit -> {
          ...
          when(req.require(MvcHandlerTest.class)).thenReturn(handler);
        })
        .expect(unit -> {
          Response rsp = unit.get(Response.class);
          when(rsp.committed()).thenReturn(false);
        ...
        .expect(unit -> {
          ...
          when(paramProvider.parameters(method)).thenReturn(params);
        })
        .run(unit -> {
  ...
  public void handleAbstractHandlers() throws Exception {
    ...
    new MockUnit(RequestParamProvider.class, Request.class, Response.class, Route.Chain.class)
        .expect(unit -> {
          Request req = unit.get(Request.class);
          when(req.require(FinalMvcHandler.class)).thenReturn(handler);
        })
        .expect(unit -> {
          Response rsp = unit.get(Response.class);
          when(rsp.committed()).thenReturn(false);
