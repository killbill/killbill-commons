...
public class Issue1087 {
  ...
  private MockUnit.Block json(String json) {
    return unit-> {
      ...
      when(ctx.accepts(MediaType.json)).thenReturn(true);
      when(ctx.type(MediaType.json)).thenReturn(ctx);
      when(ctx.length(json.length())).thenReturn(ctx);
      ctx.send(json.getBytes(StandardCharsets.UTF_8));
    };
  }
}
