...
Run tests (110 test files, 807 tests):
```
mvn clean test -pl jooby -Pjooby
```

**Note:** 15 test files that depend on PowerMock mockConstructor, Jetty 9 APIs, or external HTTP clients
are temporarily in `src/test/java-excluded/`. These will be restored after migration to Mockito
(Phase 1.7.5-1.7.6). The `-Pjooby` profile compiles and runs only the test files in `src/test/java/`.
