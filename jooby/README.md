...
Run tests (93 test files, 661 tests):
```
mvn clean test -pl jooby -Pjooby
```

**Note:** 32 test files that depend on PowerMock mockStatic/mockConstructor or external HTTP clients
are temporarily in `src/test/java-excluded/`. These will be restored after migration to Mockito
(Phase 1.7.3-1.7.6). The `-Pjooby` profile compiles and runs only the test files in `src/test/java/`.
