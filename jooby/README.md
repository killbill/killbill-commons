...
Run tests (105 test files, 751 tests):
```
mvn clean test -pl jooby -Pjooby
```

**Note:** 20 test files that depend on PowerMock mockConstructor or external HTTP clients
are temporarily in `src/test/java-excluded/`. These will be restored after migration to Mockito
(Phase 1.7.4-1.7.6). The `-Pjooby` profile compiles and runs only the test files in `src/test/java/`.
